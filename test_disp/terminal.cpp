...
void LCDTerminal::scroll() {
  ...
  _y_scroll = (_y_scroll+1)%WS_CHAR_N_Y;
  Tft.scrollAddress((INT16U)_y_scroll*WS_CHAR_S_Y);
  Tft.setFillColor(WS_BG_COLOR); 
  //bot = (WS_SCREEN_SIZE_Y+_y_scroll*WS_CHAR_S_Y)%WS_SCREEN_SIZE_Y
  Tft.fillScreen(0, WS_SCREEN_SIZE_X-1, _y_scroll, _y_scroll+WS_CHAR_S_Y);
  Tft.drawCharLowRAM('>', 0, _y_scroll);
}
