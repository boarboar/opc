...
#define SERIAL_BUFFER_SIZE_RX 120
...
unsigned char buffer_rx[SERIAL_BUFFER_SIZE_RX] = {0};
unsigned char buffer_tx[SERIAL_BUFFER_SIZE_TX] = {0};

ring_buffer rx_buffer  =  { buffer_rx, 0, 0 };
ring_buffer tx_buffer  =  { buffer_tx, 0, 0 };
#ifdef SERIAL1_AVAILABLE
#error "Not supported"
ring_buffer rx_buffer1  =  { buffer_rx, 0, 0 };
ring_buffer tx_buffer1  =  { buffer_tx, 0, 0 };
#endif

inline void store_char_rx(unsigned char c, ring_buffer *buffer)
{
	unsigned int i = (unsigned int)(buffer->head + 1) % SERIAL_BUFFER_SIZE_RX;
...
{
	unsigned int i = (_tx_buffer->head + 1) % SERIAL_BUFFER_SIZE_TX;
...
void uart_tx_isr(uint8_t offset)
{
	...
	tx_buffer_ptr->tail = (tx_buffer_ptr->tail + 1) % SERIAL_BUFFER_SIZE_TX;
	*(&(UCAxTXBUF) + offset) = c;
}
